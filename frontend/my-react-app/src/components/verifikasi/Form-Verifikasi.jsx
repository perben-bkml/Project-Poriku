...
export default function FormVerifikasi(props) {
    ...
    async function handleOnBlur(event) {
        ...
        if (searchValue !== "") {
            try {
                ...
                const response = await axios.get("http://localhost:3000/verifikasi/cari-spm", { params: { searchValue }});
    ...
    async function handleGeneratePDF() {
        try {
            setLoadingScreen(true);
            const response = await axios.post('http://localhost:3000/verifikasi/generate-pdf', {
    ...
    async function handleSubmit(event, type) {
        ...
        try {
            setLoadingScreen(true);
            const response = await axios.post("http://localhost:3000/verifikasi/verifikasi-form", { data: dataArray, type: type, rowPosition: savedRow })
            if (response.status === 200) {
                setLoadingScreen(false);
