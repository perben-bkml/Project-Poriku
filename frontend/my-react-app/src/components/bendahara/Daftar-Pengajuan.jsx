...
function DaftarPengajuan(props){
    ...
    async function fetchAntrianData (page) {
        try {
            setAntrianData([]);
            setIsLoading(true);
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/bendahara/antrian`, { params:{ page, limit: rowsPerPage, username: user.name }});
    ...
    async function handleDelPengajuan(){
        ...
        try {
            ...
            const response = await axios.delete(`${process.env.REACT_APP_API_URL}/bendahara/delete-ajuan`, { params: { tableKeyword } })
            if (response.status === 200){
                fetchAntrianData(currentPage);
                setIsAlert(true);
    ...
    async function handleFilterInputChange(event) {
        ...
        try {
            setAntrianData([]);
            setIsLoading(true);
            const response = await axios.get(`${process.env.REACT_APP_API_URL}/bendahara/filter-date`, { params:{ datePrefix, page: 1, limit: rowsPerPage }});
            if (response.status === 200){
